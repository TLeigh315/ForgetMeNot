...
def watchTemp (Q2,BLEtimer,phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude, BLEfirst_alert, BLElast_alert, base_temp, BLEbase_time, BLEstart) :
    i = 0
...
def checkMovement(q, Q2, phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude,timer, last_alert, start_program, lastx, lasty, lastz, BLEfirst_alert, BLElast_alert, BLEbase_time, base_temp):
    try:
        while (True):
            print("\r\nCurrent time is: " +str(timer))
            
            #Recieve information from the queue in the same order in was entered.
            App_phonenum = q.get()
            App_backupnum = q.get()
            App_car_color = q.get()
            App_car_type = q.get()
            App_car_license = q.get()
            App_Longitude = q.get()
            App_Latitude = q.get()
            ...
            if (App_Latitude == "\r" and App_Longitude == "\r"): #Disconnected from App begin Temp/Timer alert procedure
...
##                q.put(App_Latitude)
##                q.put(App_Longitude)
                checktemp = watchTemp(Q2,timer, phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude, BLEfirst_alert, BLElast_alert, base_temp, BLEbase_time, BLEstart)
...
def connected2App(q, Q2 ,phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude):  
    while (True):
        ...
        if (address == ('50:77:05:A7:B3:99', 1)): #if connected to phone
            ...
            while (True):
                try:
                    assigndata(client_sock.recv(1024),phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude)

                    App_phonenum = AppData['phonenum']
                    App_backupnum = AppData['backupnum']
                    App_car_color = AppData['car_color']
                    App_car_type = AppData['car_type']
                    App_car_license = AppData['car_license']
                    App_Longitude = AppData['Longitude']
                    App_Latitude = AppData['Latitude']

                    #Place information into queue to pass to other thread. Order of info matters!
                    if (App_Latitude == "\r" and App_Longitude == "\r"): #Disconnected from App begin Temp/Timer alert procedure
                        Q2.put(AppData['Longitude'])
                        Q2.put(AppData['Latitude'])
                    else:
                        q.put(AppData['phonenum'])
                        q.put(AppData['backupnum'])
                        q.put(AppData['car_color'])
                        q.put(AppData['car_type'])
                        q.put(AppData['car_license'])
                        q.put(AppData['Longitude'])
                        q.put(AppData['Latitude'])
...
##        AppData = assigndata("P42872945725\r\nCBlack\r\nTFord Escape\r\nLLDAKSF425\r\nB324542452\r\n",phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude)  
##
##        #Place information into queue to pass to other thread. Order of info matters!
##        q.put(AppData['phonenum'])
##        q.put(AppData['backupnum'])
...
try:
    queue = Queue()
    queue2 = Queue()
    thread1 = Thread( target=checkMovement, args=(queue, queue2, phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude, timer, last_alert, start_program, lastx, lasty, lastz, BLEfirst_alert, BLElast_alert, BLEbase_time, base_temp) )
    thread2 = Thread( target=connected2App, args=(queue, queue2, phonenum,backupnum,car_color,car_type,car_license,Longitude,Latitude) )
