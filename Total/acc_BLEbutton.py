...
accelsensor = LIS3DH.LIS3DH() #Create LIS3DH accelerometer
###########################################################################################
...
while True:
    if (GPIO.input(BLEonbutton) == 1 ) & ( GPIO.input(BLEoffbutton) == 0 ): #if ONLY the onbutton is pushed
        ...
        while (GPIO.input(BLEoffbutton) == 0) :# offbutton isn't pushed (parent hasn't returned)
            ...
            temperature = tempsensor.Temperature(base_temp, BLEbase_time, BLEtimer, start, danger_rate, last_alert, ser, max)
            ...
            if (BLEtimer-first_alert) == 4*60 : #if it's been 4 min since first temp alert call EMS
